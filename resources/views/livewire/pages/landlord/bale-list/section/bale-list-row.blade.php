<tr wire:key="bale-list-row-{{ $record->id }}"
    ...
    <td class="py-4 pl-4 pr-3 text-sm font-medium text-gray-900 max-w-0 sm:w-auto sm:max-w-none">
        <button wire:click="$parent.selectBale('{{ $record->id }}')"
            class="flex cursor-pointer items-center gap-x-3 text-left w-full group">
            <div
            ...
            <div>
                <span
                    class="block text-sm text-gray-800 dark:text-gray-200 font-bold group-hover:text-indigo-600 dark:group-hover:text-indigo-400 decoration-indigo-500/30 underline-offset-4 group-hover:underline transition-colors">{{ $record->name }}</span>
    ...
    <td class="px-3 py-4 text-sm text-gray-500">
        @if($record->is_active)
            <span
                class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-medium bg-green-50 text-green-700 ring-1 ring-inset ring-green-600/20 dark:bg-green-900/30 dark:text-green-400">
    ...
    @canany(['bale-list.update', 'bale-list.delete'])
        <td class="px-6 py-1.5 whitespace-nowrap w-px">
            <livewire:core.shared-components.item-actions :editUrl="route('rakaca.landlord.bale-list.edit', $record->id)"
                :deleteId="$record->id" :navigate="false" wire:key="item-actions-{{ $record->id }}"
