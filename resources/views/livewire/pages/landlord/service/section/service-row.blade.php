<tr wire:key="service-row-{{ $record->id }}"
    ...
    <td class="py-4 pl-4 pr-3 text-sm font-medium text-gray-900 max-w-0 sm:w-auto sm:max-w-none">
        <div class="flex items-center gap-x-3">
            <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 font-semibold p-2 shadow-sm ring-1 ring-inset ring-indigo-500/10">
                @if($record->icon)
                    <x-dynamic-component :component="'lucide-' . $record->icon" class="w-5 h-5" />
                @else
    ...
    @canany(['service.update', 'service.delete'])
        <td class="px-6 py-1.5 whitespace-nowrap w-px">
            <livewire:core.shared-components.item-actions
                :editUrl="route('rakaca.landlord.service.edit', $record->id)" 
                :deleteId="$record->id"
                :navigate="false"
